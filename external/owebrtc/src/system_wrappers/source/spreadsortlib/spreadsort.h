...
namespace boost {
  ...
  namespace detail {
  	...
  	inline void
  	update_offset(RandomAccessIter first, RandomAccessIter finish, unsigned &char_offset, get_char _GETCHAR, get_length length)
  	{
  		unsigned nextOffset = char_offset;
  		bool done = false;
  		while(!done) {
  			...
  			do {
  				//ignore empties, but if the nextOffset would exceed the length or not match, exit; we've found the last matching character
  				if(length(*curr) > char_offset && (length(*curr) <= (nextOffset + 1) || _GETCHAR((*curr), nextOffset) != _GETCHAR((*first), nextOffset))) {
  					done = true;
  					break;
  				}
  			} while(++curr != finish);
  	...
  	struct offset_char_lessthan {
  		...
  		inline bool operator()(const data_type &x, const data_type &y) const 
  		{
  			...
  			for(unsigned u = fchar_offset; u < minSize; ++u) {
  				if(_GETCHAR(x, u) < _GETCHAR(y, u))
  					return true;
  				else if(_GETCHAR(y, u) < _GETCHAR(x, u))
  					return false;
  			}
  	...
  	inline void 
  	string_sort_rec(RandomAccessIter first, RandomAccessIter last, unsigned char_offset, std::vector<RandomAccessIter> &bin_cache
  		, unsigned cache_offset, std::vector<size_t> &bin_sizes, get_char _GETCHAR, get_length length)
  	{
  		//This section is not strictly necessary, but makes handling of long identical substrings much faster, with a mild average performance impact.
  		//Iterate to the end of the empties.  If all empty, return
  		...
  		++finish;
  		update_offset(first, finish, char_offset, _GETCHAR, length);
  		...
  		for (RandomAccessIter current = first; current != last; ++current) {
  			if(length(*current) <= char_offset) {
  				...
  				bin_sizes[_GETCHAR((*current), char_offset) + 1]++;
  		}
  		//Assign the bin positions
  		bin_cache[cache_offset] = first;
  		for(unsigned u = 0; u < membin_count - 1; u++)
  		...
  		for(RandomAccessIter current = *local_bin; current < nextbinstart; ++current) {
  			...
  			while(length(*current) > char_offset) {
  				target_bin = bins + _GETCHAR((*current), char_offset);
  				iter_swap(current, (*target_bin)++);
  			}
  		}
  		*local_bin = nextbinstart;
  		...
  		for(unsigned ii = 0; ii < last_bin; ++ii) {
  			...
  			for(RandomAccessIter current = *local_bin; current < nextbinstart; ++current) {
  				//Swapping elements in current into place until the correct element has been swapped in
  				for(target_bin = bins + _GETCHAR((*current), char_offset);  target_bin != local_bin; 
  					target_bin = bins + _GETCHAR((*current), char_offset))
  					iter_swap(current, (*target_bin)++);
  			}
  			*local_bin = nextbinstart;
  		...
  		for(unsigned u = cache_offset + 1; u < cache_offset + last_bin + 2; lastPos = bin_cache[u], ++u) {
  			...
  			if(count < max_size)
  				...
  				string_sort_rec<RandomAccessIter, data_type, unsignedchar_type, get_char, get_length>(lastPos, bin_cache[u], char_offset + 1, bin_cache, cache_end, bin_sizes, _GETCHAR, length);
  		}
  	}
  	...
  	inline void 
  	string_sort_rec(RandomAccessIter first, RandomAccessIter last, unsigned char_offset, std::vector<RandomAccessIter> &bin_cache
  		, unsigned cache_offset, std::vector<size_t> &bin_sizes, get_char _GETCHAR, get_length length, compare comp)
  	{
  		//This section is not strictly necessary, but makes handling of long identical substrings much faster, with a mild average performance impact.
  		//Iterate to the end of the empties.  If all empty, return
  		while(length(*first) <= char_offset) {
  		...
  		for(unsigned u = cache_offset + 1; u < cache_offset + last_bin + 2; lastPos = bin_cache[u], ++u) {
  			...
  			if(count < max_size)
  				...
  				string_sort_rec<RandomAccessIter, data_type, unsignedchar_type, get_char, get_length, compare>(lastPos
  					, bin_cache[u], char_offset + 1, bin_cache, cache_end, bin_sizes, _GETCHAR, length, comp);
  		}
  	}
  		...
  		, unsigned cache_offset, std::vector<size_t> &bin_sizes, get_char _GETCHAR, get_length length, compare comp)
  	{
  		...
  		++last;
  		//Offsetting on identical characters.  This section works a character at a time for optimal worst-case performance.
  		update_offset(first, last, char_offset, _GETCHAR, length);
  		...
  		for (RandomAccessIter current = first; current != last; ++current) {
  			if(length(*current) <= char_offset) {
  				...
  				bin_sizes[max_bin - _GETCHAR((*current), char_offset)]++;
  		}
  		//Assign the bin positions
  		bin_cache[cache_offset] = first;
  		for(unsigned u = 0; u < membin_count - 1; u++)
  		...
  		for(RandomAccessIter current = *local_bin; current < nextbinstart; ++current) {
  			//empties belong in this bin
  			while(length(*current) > char_offset) {
  				target_bin = end_bin - _GETCHAR((*current), char_offset);
  				iter_swap(current, (*target_bin)++);
  			}
  		}
  		*local_bin = nextbinstart;
  		...
  		for(unsigned u = 0; u < last_bin; ++u) {
  			...
  			for(RandomAccessIter current = *local_bin; current < nextbinstart; ++current) {
  				//Swapping elements in current into place until the correct element has been swapped in
  				for(target_bin = end_bin - _GETCHAR((*current), char_offset);  target_bin != local_bin; 
  					target_bin = end_bin - _GETCHAR((*current), char_offset))
  					iter_swap(current, (*target_bin)++);
  			}
  			*local_bin = nextbinstart;
  	...
  	inline void 
  	string_sort(RandomAccessIter first, RandomAccessIter last, get_char _GETCHAR, get_length length, data_type, unsignedchar_type)
  	{
  		std::vector<size_t> bin_sizes;
  		std::vector<RandomAccessIter> bin_cache;
  		string_sort_rec<RandomAccessIter, data_type, unsignedchar_type, get_char, get_length>(first, last, 0, bin_cache, 0, bin_sizes, _GETCHAR, length);
  	}

  	//Holds the bin vector and makes the initial recursive call
  	template <class RandomAccessIter, class get_char, class get_length, class compare, class data_type, class unsignedchar_type>
  	inline void 
  	string_sort(RandomAccessIter first, RandomAccessIter last, get_char _GETCHAR, get_length length, compare comp, data_type, unsignedchar_type)
  	{
  		std::vector<size_t> bin_sizes;
  		std::vector<RandomAccessIter> bin_cache;
  		string_sort_rec<RandomAccessIter, data_type, unsignedchar_type, get_char, get_length, compare>(first, last, 0, bin_cache, 0, bin_sizes, _GETCHAR, length, comp);
  	}

  	//Holds the bin vector and makes the initial recursive call
  	template <class RandomAccessIter, class get_char, class get_length, class compare, class data_type, class unsignedchar_type>
  	inline void 
  	reverse_string_sort(RandomAccessIter first, RandomAccessIter last, get_char _GETCHAR, get_length length, compare comp, data_type, unsignedchar_type)
  	{
  		std::vector<size_t> bin_sizes;
  		std::vector<RandomAccessIter> bin_cache;
  		reverse_string_sort_rec<RandomAccessIter, data_type, unsignedchar_type, get_char, get_length, compare>(first, last, 0, bin_cache, 0, bin_sizes, _GETCHAR, length, comp);
  	}
  }
  ...
  template <class RandomAccessIter, class get_char, class get_length>
  inline void string_sort(RandomAccessIter first, RandomAccessIter last, get_char _GETCHAR, get_length length) 
  {
  	//Don't sort if it's too small to optimize
  	if(last - first < detail::MIN_SORT_SIZE)
  		std::sort(first, last);
  		...
  		detail::string_sort(first, last, _GETCHAR, length, *first, _GETCHAR((*first), 0));
  	}
  }

  template <class RandomAccessIter, class get_char, class get_length, class compare>
  inline void string_sort(RandomAccessIter first, RandomAccessIter last, get_char _GETCHAR, get_length length, compare comp) 
  {
  	//Don't sort if it's too small to optimize
  	if(last - first < detail::MIN_SORT_SIZE)
  		std::sort(first, last, comp);
  		...
  		detail::string_sort(first, last, _GETCHAR, length, comp, *first, _GETCHAR((*first), 0));
  	}
  }

  template <class RandomAccessIter, class get_char, class get_length, class compare>
  inline void reverse_string_sort(RandomAccessIter first, RandomAccessIter last, get_char _GETCHAR, get_length length, compare comp) 
  {
  	//Don't sort if it's too small to optimize
  	if(last - first < detail::MIN_SORT_SIZE)
  		std::sort(first, last, comp);
  		...
  		++last;
  		detail::reverse_string_sort(first, last, _GETCHAR, length, comp, *first, _GETCHAR((*first), 0));
  	}
  }
}
